...
function buildOverpassQuery(tags, lat, lng, radius, limit) {
  const parts = tags.map(([k,v]) => `  node["${k}"="${v}"](around:${radius},${lat},${lng});\n  way["${k}"="${v}"](around:${radius},${lat},${lng});\n`).join("");
  return `[out:json][timeout:30];\n(\n${parts});\nout center ${limit};`;
}

function parseOsmHours(ohString) {
  if (!ohString) return null;
  if (ohString === "24/7") return Array.from({length:7},(_,i)=>({day_of_week:i,open_time:"00:00",close_time:"23:59",is_closed:false}));
  const DAY_MAP = {"Mo":[1],"Tu":[2],"We":[3],"Th":[4],"Fr":[5],"Sa":[6],"Su":[0],"Mo-Fr":[1,2,3,4,5],"Mo-Sa":[1,2,3,4,5,6],"Mo-Su":[0,1,2,3,4,5,6],"Tu-Fr":[2,3,4,5],"Sa-Su":[6,0]};
  const result = {};
  for (let i=0;i<7;i++) result[i]={open:null,close:null,closed:true};
  for (const seg of ohString.split(";").map(s=>s.trim())) {
...
export async function GET() {
  return NextResponse.json({status:"ok",source:"OpenStreetMap / Overpass API",note:"POST {category,city,count} pour lancer.",cities:Object.keys(CITY_COORDS),categories:Object.keys(CATEGORY_OSM)});
}

export async function POST(req) {
  const body = await req.json();
  const category = body.category||"loisirs";
  const city = body.city||"Tunis";
  ...
  const existingNoms = new Set((existingData||[]).map(l=>l.nom.toLowerCase().trim()));
  ...
  for (const el of unique.slice(0,count)) {
    try {
      ...
      const adresse=[tags["addr:housenumber"],tags["addr:street"],tags["addr:suburb"]].filter(Boolean).join(" ")||tags["addr:full"]||null;
    ...
    } catch(e){errors.push(`Erreur: ${e.message}`);}
  }
